...
def apply_runtime_defaults(args, is_ogb: bool, impl: str) -> None:
    ...
    if args.mini_batch:
        ...
        if bool(args.use_homo_feature_boost):
...
def train_getsic_fullbatch_epoch(model: nn.Module, data: Data, args, epoch: int, opt, scheduler) -> None:
    ...
    loss.backward()
    if not finite_gradients(model):
        print('[warn] non-finite GETSIC gradient detected; skipping step')
        opt.zero_grad(set_to_none=True)
        return
    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0, error_if_nonfinite=False)
    opt.step()
...
def train_getsic_minibatch_epoch(model: nn.Module, data: Data, args, device: torch.device, epoch: int, opt, scheduler) -> None:
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        loss.backward()
        if not finite_gradients(model):
            print('[warn] non-finite GETSIC mini-batch gradient detected; skipping step')
            opt.zero_grad(set_to_none=True)
            continue
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0, error_if_nonfinite=False)
        opt.step()
...
def run_getsic(raw_args, provided: Optional[set] = None) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.mini_batch:
            train_getsic_minibatch_epoch(model, data, args, device, epoch, opt, scheduler)
        else:
            train_getsic_fullbatch_epoch(model, data, args, epoch, opt, scheduler)
        stats = end_epoch_profile(device, start, start_cpu)
        print_epoch_cost('GETSIC', epoch, stats)
...
def train_ascii_fullbatch_epoch(model: nn.Module, data: Data, args, epoch: int, opt, scheduler) -> None:
    ...
    loss.backward()
    if not finite_gradients(model):
        print('[warn] non-finite ASCii gradient detected; skipping step')
        opt.zero_grad(set_to_none=True)
        return
    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0, error_if_nonfinite=False)
    opt.step()
...
def run_ascii(raw_args, provided: Optional[set] = None) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        start, start_cpu = begin_epoch_profile(device)
        if args.mini_batch:
            train_minibatch_epoch(model, data, args, device, epoch, opt, scheduler)
        else:
            train_ascii_fullbatch_epoch(model, data, args, epoch, opt, scheduler)
        stats = end_epoch_profile(device, start, start_cpu)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--impl', type=str, default='getsic', choices=['getsic', 'ascii', 'both'], help='Which implementation to run. both runs GETSIC first, then ASCii.')
    ...
    parser.add_argument('--base_model', '--base-model', dest='base_model', type=str, default='auto', choices=['auto', 'adaptive', 'gesc'], help='ASCii only: auto uses non-adaptive GESC on Cora and adaptive ASCii elsewhere.')
    ...
    parser.add_argument('--sic_first', type=float, default=1.5, help='GETSIC / GESC fixed first-layer SIC strength.')
    ...
    parser.add_argument('--eta_max', type=float, default=0.75, help='ASCii only: upper bound for adaptive SIC coefficient.')
    parser.add_argument('--eta_bar', type=float, default=0.5, help='ASCii only: center for eta regularization.')
    parser.add_argument('--eta_reg', type=float, default=0.0001, help='ASCii only: weight for coefficient stabilizer.')
    parser.add_argument('--lambda_attn', type=float, default=0.5, help='ASCii only: mix between magnitude attention and signed phase attention.')
    parser.add_argument('--readout_rank', type=int, default=16, help='ASCii only: rank r for magnitude-Gram readout.')
    parser.add_argument('--readout_hidden', type=int, default=0, help='ASCii only: hidden width for readout MLP; 0 uses linear classifier.')
    parser.add_argument('--use_phase', action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument('--phase_scale', type=float, default=1.0, help='ASCii only: theta range is +/- pi * phase_scale.')
    ...
    parser.add_argument('--patience', type=int, default=200, help='Kept for CLI compatibility; not used because no validation is printed/performed.')
    parser.add_argument('--consistency_w', type=float, default=0.1)
    parser.add_argument('--cons_T', type=float, default=2.0)
    parser.add_argument('--use_cs', action=argparse.BooleanOptionalAction, default=True, help='Kept for CLI compatibility; no performance evaluation is run.')
    ...
    parser.add_argument('--use_lp', action=argparse.BooleanOptionalAction, default=True, help='Kept for CLI compatibility; no performance evaluation is run.')
    ...
    parser.add_argument('--eval_batch_size', type=int, default=None, help='Kept for CLI compatibility.')
    ...
    parser.add_argument('--eval_num_neighbors', type=str, default=None, help='Kept for CLI compatibility.')
    parser.add_argument('--num_workers', type=int, default=0, help='NeighborLoader worker count. Use 0 if sampler workers have issues.')
    ...
    parser.add_argument('--eval_train', action=argparse.BooleanOptionalAction, default=True, help='Kept for CLI compatibility.')
    parser.add_argument('--attn_mode', type=str, default='auto', choices=['auto', 'softmax', 'sigmoid_degree', 'degree'], help='ASCii only: softmax original attention; sigmoid_degree memory-safe OGB default.')
