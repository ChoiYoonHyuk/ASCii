...
def nan_to_num_safe(x: Tensor, nan: float = 0.0, posinf: float = 0.0, neginf: float = 0.0) -> Tensor:
    ...
    """torch.nan_to_num that also handles complex tensors."""
...
class ModReLU(nn.Module):
    ...
    """Phase-preserving complex modReLU."""
    ...
    def forward(self, z: Tensor) -> Tensor:
        ...
        mag = torch.abs(z).clamp_min(1e-6)
...
class PhasePreservingNodeNorm(nn.Module):
    ...
    """
    Node-wise RMS normalization for complex states.

    This differs from the older coordinate-wise NodeNorm. It rescales by a real
    scalar computed from magnitudes, so it does not choose a real/imag frame.
    """
...
def compute_local_inconsistency(u: Tensor, edge_index: Tensor, num_nodes: int, eps: float = 1e-12) -> Tensor:
    ...
    """
    q_i = 1 - mean_{k in N(i)} cos(u_i, u_k), using only features or frozen unary beliefs.

    Self-loops are removed here so q_i is not dominated by cos(u_i, u_i)=1.
    """
...
class ASCiiLayer(nn.Module):
    ...
    """
    One adaptive self-interference control layer.

    edge_index convention: src=edge_index[0] is j, dst=edge_index[1] is i.
    """
    ...
    def forward(
        ...
        delta_x, delta_d, qi, qj, layer_frac = self._edge_static_terms(
        ...
        for m in range(self.M):
            ...
            nu = torch.linalg.vector_norm(Qhi, dim=-1, keepdim=True) * torch.linalg.vector_norm(

                transported, dim=-1, keepdim=True
            ...
            denom_r = torch.linalg.vector_norm(Qhi, dim=-1, keepdim=True) * torch.linalg.vector_norm(
            ...
            denom_hat = torch.linalg.vector_norm(Qhi, dim=-1, keepdim=True) * torch.linalg.vector_norm(

                msg_hat, dim=-1, keepdim=True
    ...
    def forward(self, h: Tensor) -> Tensor:
        ...
        phi = torch.cat(
            ...
            [
                ...
                gram.real.reshape(h.size(0), -1),

                gram.imag.reshape(h.size(0), -1),
...
class ASCiiNet(nn.Module):
    ...
    def __init__(
        ...
        self.readout = InvariantGramReadout(

            complex_dim=readout_dim,
    ...
    def prepare_static_graph(
        ...
        unary_view: Optional[Tensor] = None,

    ) -> None:

        """
        Precompute label-safe node descriptors.

        unary_view can be frozen feature-only predictions or embeddings. If omitted,
        normalized input features are used as the unsupervised proxy for q_i.
        """
...
def correct_and_smooth_compat(
    ...
    cs_corr_layers: int = 50,

    cs_corr_alpha: float = 0.5,

    cs_smooth_layers: int = 50,

    cs_smooth_alpha: float = 0.8,
    ...
    with torch.no_grad():
        ...
        cs = CorrectAndSmooth(
            ...
            num_correction_layers=cs_corr_layers,

            correction_alpha=cs_corr_alpha,

            num_smoothing_layers=cs_smooth_layers,

            smoothing_alpha=cs_smooth_alpha,
...
def evaluate(model: nn.Module, data, args) -> Tuple[float, float, float, Tensor]:
    ...
    if args.use_cs:
        ...
        y_smooth = correct_and_smooth_compat(
            ...
            cs_corr_layers=args.cs_corr_layers,

            cs_corr_alpha=args.cs_corr_alpha,

            cs_smooth_layers=args.cs_smooth_layers,

            cs_smooth_alpha=args.cs_smooth_alpha,
...
def train_main() -> None:
    ...
    parser.add_argument(
        ...
        help="0=Cora,1=Citeseer,2=Pubmed,3=Chameleon,4=Squirrel,5=Actor,6=Cornell,7=Texas,else=Wisconsin",
    ...
    parser.add_argument("--eta_reg", type=float, default=1e-4, help="Weight for coefficient stabilizer.")
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        ce = cross_entropy_with_label_smoothing(
            ...
            logits1[data.train_mask], data.y[data.train_mask], smoothing=args.label_smooth
        ...
        print(
            ...
            f"[{epoch}] loss={loss.item():.4f} ce={ce.item():.4f} cons={cons.item():.4f} eta={eta_loss.item():.4f} | "

            f"train={train_acc:.3f} val={val_acc:.3f} test={test_acc:.3f} | "

            f"best@val(epoch={best_epoch})={best_val_acc:.3f}/{test_acc_at_best_val:.3f}"
